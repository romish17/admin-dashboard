...
import { Plus, Search, Server, Pencil } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Zabbix Items</h1>
          <p className="text-muted-foreground">Manage Zabbix templates, triggers, and configurations</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="card text-center py-12">
          ...
          <p className="text-muted-foreground">No Zabbix items found.</p>
        </div>
      ) : (
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3">
                    <h3 className="text-lg font-medium text-foreground group-hover:text-primary">{item.name}</h3>
                    <span className={cn('badge', typeColors[item.itemType])}>{item.itemType}</span>
                    <button
                  ...
                  {item.zabbixId && (
                    <p className="text-muted-foreground text-xs mt-2 font-mono">Zabbix ID: {item.zabbixId}</p>
                  )}
                </div>
              </div>
            </div>
